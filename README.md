...
> 
> **项目取得部分成功，但因个人设备与能力所限只能进行到这里。期望未来有人继续。**
...
主流大语言模型（GPT、Claude 等）需要数千亿参数和海量数据，普通人既无设备也无资源复现。本项目从相反方向切入：**舍弃反向传播和全局梯度，采用纯脉冲神经网络（SNN）和局部学习规则（STDP），探索在低参数成本（仅 10,000 神经元、1,000,000 突触）下，能否涌现出自发学习能力乃至语义理解。**

实验产出了部分成功——纯 STDP 确实让突触权重形成了双峰分布，网络自发实现了稀疏化和兴奋/抑制平衡。但碍于创作者现实中的个人能力与设备限制（单卡 RTX 4060，仅能承载 10⁴ 神经元规模），所有实验研究只能进行到这里。

**这不是一个完成的成果，而是一个需要被继续的探索。** 希望未来有人能在更大规模、更丰富机制的条件下延续这个方向——验证 SNN 在合理规模下能否跨越从"突触结构"到"语义结构"的鸿沟。

这不是"用 SNN 复刻 GPT"的项目，而是一次**对纯局部学习规则能力边界的科学探查**——并且它远未结束。
